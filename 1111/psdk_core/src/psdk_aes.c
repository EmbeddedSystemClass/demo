...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
// AES operates on 16 bytes at a time , can not change by user
#define PSDK_AES_BLOCK_SIZE 16
...
static const uint8_t s_aesSbox[16][16] = {
...
static const uint8_t s_aesInvsbox[16][16] = {
...
static const uint8_t s_gfMul[256][6] = {
...
//static void PsdkAes_XorBuf(uint8_t const *in, uint8_t *out, size_t len)
//{
//    size_t idx;
...
static uint32_t PsdkAes_SubWord(uint32_t word)
{
    unsigned int result;

    result = (int) s_aesSbox[(word >> 4) & 0x0000000F][word & 0x0000000F];
    result += (int) s_aesSbox[(word >> 12) & 0x0000000F][(word >> 8) & 0x0000000F] << 8;
    result += (int) s_aesSbox[(word >> 20) & 0x0000000F][(word >> 16) & 0x0000000F] << 16;
    result += (int) s_aesSbox[(word >> 28) & 0x0000000F][(word >> 24) & 0x0000000F] << 24;
    return (result);
}
...
static void PsdkAes_KeySetup(const uint8_t *key, uint32_t *w, int keySize)
{
    int nb = 4, nr, nk, idx;
    ...
    for (idx = nk; idx < nb * (nr + 1); ++idx) {
        temp = w[idx - 1];
        if ((idx % nk) == 0)
            temp = PsdkAes_SubWord(KE_ROTWORD(temp)) ^ rcon[(idx - 1) / nk];
        else if (nk > 6 && (idx % nk) == 4)
            temp = PsdkAes_SubWord(temp);
        w[idx] = w[idx - nk] ^ temp;
    }
}
...
static void PsdkAes_AddRoundKey(uint8_t state[4][4], const uint32_t *w)
{
    uint8_t subKey[4];
...
static void PsdkAes_SubBytes(uint8_t state[4][4])
{
    state[0][0] = s_aesSbox[state[0][0] >> 4][state[0][0] & 0x0F];
    state[0][1] = s_aesSbox[state[0][1] >> 4][state[0][1] & 0x0F];
    state[0][2] = s_aesSbox[state[0][2] >> 4][state[0][2] & 0x0F];
    state[0][3] = s_aesSbox[state[0][3] >> 4][state[0][3] & 0x0F];
    state[1][0] = s_aesSbox[state[1][0] >> 4][state[1][0] & 0x0F];
    state[1][1] = s_aesSbox[state[1][1] >> 4][state[1][1] & 0x0F];
    state[1][2] = s_aesSbox[state[1][2] >> 4][state[1][2] & 0x0F];
    state[1][3] = s_aesSbox[state[1][3] >> 4][state[1][3] & 0x0F];
    state[2][0] = s_aesSbox[state[2][0] >> 4][state[2][0] & 0x0F];
    state[2][1] = s_aesSbox[state[2][1] >> 4][state[2][1] & 0x0F];
    state[2][2] = s_aesSbox[state[2][2] >> 4][state[2][2] & 0x0F];
    state[2][3] = s_aesSbox[state[2][3] >> 4][state[2][3] & 0x0F];
    state[3][0] = s_aesSbox[state[3][0] >> 4][state[3][0] & 0x0F];
    state[3][1] = s_aesSbox[state[3][1] >> 4][state[3][1] & 0x0F];
    state[3][2] = s_aesSbox[state[3][2] >> 4][state[3][2] & 0x0F];
    state[3][3] = s_aesSbox[state[3][3] >> 4][state[3][3] & 0x0F];
}

static void PsdkAes_InvSubBytes(uint8_t state[4][4])
{
    state[0][0] = s_aesInvsbox[state[0][0] >> 4][state[0][0] & 0x0F];
    state[0][1] = s_aesInvsbox[state[0][1] >> 4][state[0][1] & 0x0F];
    state[0][2] = s_aesInvsbox[state[0][2] >> 4][state[0][2] & 0x0F];
    state[0][3] = s_aesInvsbox[state[0][3] >> 4][state[0][3] & 0x0F];
    state[1][0] = s_aesInvsbox[state[1][0] >> 4][state[1][0] & 0x0F];
    state[1][1] = s_aesInvsbox[state[1][1] >> 4][state[1][1] & 0x0F];
    state[1][2] = s_aesInvsbox[state[1][2] >> 4][state[1][2] & 0x0F];
    state[1][3] = s_aesInvsbox[state[1][3] >> 4][state[1][3] & 0x0F];
    state[2][0] = s_aesInvsbox[state[2][0] >> 4][state[2][0] & 0x0F];
    state[2][1] = s_aesInvsbox[state[2][1] >> 4][state[2][1] & 0x0F];
    state[2][2] = s_aesInvsbox[state[2][2] >> 4][state[2][2] & 0x0F];
    state[2][3] = s_aesInvsbox[state[2][3] >> 4][state[2][3] & 0x0F];
    state[3][0] = s_aesInvsbox[state[3][0] >> 4][state[3][0] & 0x0F];
    state[3][1] = s_aesInvsbox[state[3][1] >> 4][state[3][1] & 0x0F];
    state[3][2] = s_aesInvsbox[state[3][2] >> 4][state[3][2] & 0x0F];
    state[3][3] = s_aesInvsbox[state[3][3] >> 4][state[3][3] & 0x0F];
}

// Performs the ShiftRows step. All rows are shifted cylindrically to the left.
static void PsdkAes_ShiftRows(uint8_t state[4][4])
{
    uint8_t temp;
...
static void PsdkAes_InvShiftRows(uint8_t state[4][4])
{
    uint8_t temp;
...
static void PsdkAes_MixColumns(uint8_t state[4][4])
{
    uint8_t col[4];
    ...
    col[2] = state[2][0];
    col[3] = state[3][0];
    state[0][0] = s_gfMul[col[0]][0];
    state[0][0] ^= s_gfMul[col[1]][1];
    state[0][0] ^= col[2];
    state[0][0] ^= col[3];
    state[1][0] = col[0];
    state[1][0] ^= s_gfMul[col[1]][0];
    state[1][0] ^= s_gfMul[col[2]][1];
    state[1][0] ^= col[3];
    state[2][0] = col[0];
    state[2][0] ^= col[1];
    state[2][0] ^= s_gfMul[col[2]][0];
    state[2][0] ^= s_gfMul[col[3]][1];
    state[3][0] = s_gfMul[col[0]][1];
    state[3][0] ^= col[1];
    state[3][0] ^= col[2];
    state[3][0] ^= s_gfMul[col[3]][0];
    // Column 2
    col[0] = state[0][1];
    col[1] = state[1][1];
    col[2] = state[2][1];
    col[3] = state[3][1];
    state[0][1] = s_gfMul[col[0]][0];
    state[0][1] ^= s_gfMul[col[1]][1];
    state[0][1] ^= col[2];
    state[0][1] ^= col[3];
    state[1][1] = col[0];
    state[1][1] ^= s_gfMul[col[1]][0];
    state[1][1] ^= s_gfMul[col[2]][1];
    state[1][1] ^= col[3];
    state[2][1] = col[0];
    state[2][1] ^= col[1];
    state[2][1] ^= s_gfMul[col[2]][0];
    state[2][1] ^= s_gfMul[col[3]][1];
    state[3][1] = s_gfMul[col[0]][1];
    state[3][1] ^= col[1];
    state[3][1] ^= col[2];
    state[3][1] ^= s_gfMul[col[3]][0];
    // Column 3
    col[0] = state[0][2];
    col[1] = state[1][2];
    col[2] = state[2][2];
    col[3] = state[3][2];
    state[0][2] = s_gfMul[col[0]][0];
    state[0][2] ^= s_gfMul[col[1]][1];
    state[0][2] ^= col[2];
    state[0][2] ^= col[3];
    state[1][2] = col[0];
    state[1][2] ^= s_gfMul[col[1]][0];
    state[1][2] ^= s_gfMul[col[2]][1];
    state[1][2] ^= col[3];
    state[2][2] = col[0];
    state[2][2] ^= col[1];
    state[2][2] ^= s_gfMul[col[2]][0];
    state[2][2] ^= s_gfMul[col[3]][1];
    state[3][2] = s_gfMul[col[0]][1];
    state[3][2] ^= col[1];
    state[3][2] ^= col[2];
    state[3][2] ^= s_gfMul[col[3]][0];
    // Column 4
    col[0] = state[0][3];
    col[1] = state[1][3];
    col[2] = state[2][3];
    col[3] = state[3][3];
    state[0][3] = s_gfMul[col[0]][0];
    state[0][3] ^= s_gfMul[col[1]][1];
    state[0][3] ^= col[2];
    state[0][3] ^= col[3];
    state[1][3] = col[0];
    state[1][3] ^= s_gfMul[col[1]][0];
    state[1][3] ^= s_gfMul[col[2]][1];
    state[1][3] ^= col[3];
    state[2][3] = col[0];
    state[2][3] ^= col[1];
    state[2][3] ^= s_gfMul[col[2]][0];
    state[2][3] ^= s_gfMul[col[3]][1];
    state[3][3] = s_gfMul[col[0]][1];
    state[3][3] ^= col[1];
    state[3][3] ^= col[2];
    state[3][3] ^= s_gfMul[col[3]][0];
}

static void PsdkAes_InvMixColumns(uint8_t state[4][4])
{
    uint8_t col[4];
    ...
    col[2] = state[2][0];
    col[3] = state[3][0];
    state[0][0] = s_gfMul[col[0]][5];
    state[0][0] ^= s_gfMul[col[1]][3];
    state[0][0] ^= s_gfMul[col[2]][4];
    state[0][0] ^= s_gfMul[col[3]][2];
    state[1][0] = s_gfMul[col[0]][2];
    state[1][0] ^= s_gfMul[col[1]][5];
    state[1][0] ^= s_gfMul[col[2]][3];
    state[1][0] ^= s_gfMul[col[3]][4];
    state[2][0] = s_gfMul[col[0]][4];
    state[2][0] ^= s_gfMul[col[1]][2];
    state[2][0] ^= s_gfMul[col[2]][5];
    state[2][0] ^= s_gfMul[col[3]][3];
    state[3][0] = s_gfMul[col[0]][3];
    state[3][0] ^= s_gfMul[col[1]][4];
    state[3][0] ^= s_gfMul[col[2]][2];
    state[3][0] ^= s_gfMul[col[3]][5];
    // Column 2
    col[0] = state[0][1];
    col[1] = state[1][1];
    col[2] = state[2][1];
    col[3] = state[3][1];
    state[0][1] = s_gfMul[col[0]][5];
    state[0][1] ^= s_gfMul[col[1]][3];
    state[0][1] ^= s_gfMul[col[2]][4];
    state[0][1] ^= s_gfMul[col[3]][2];
    state[1][1] = s_gfMul[col[0]][2];
    state[1][1] ^= s_gfMul[col[1]][5];
    state[1][1] ^= s_gfMul[col[2]][3];
    state[1][1] ^= s_gfMul[col[3]][4];
    state[2][1] = s_gfMul[col[0]][4];
    state[2][1] ^= s_gfMul[col[1]][2];
    state[2][1] ^= s_gfMul[col[2]][5];
    state[2][1] ^= s_gfMul[col[3]][3];
    state[3][1] = s_gfMul[col[0]][3];
    state[3][1] ^= s_gfMul[col[1]][4];
    state[3][1] ^= s_gfMul[col[2]][2];
    state[3][1] ^= s_gfMul[col[3]][5];
    // Column 3
    col[0] = state[0][2];
    col[1] = state[1][2];
    col[2] = state[2][2];
    col[3] = state[3][2];
    state[0][2] = s_gfMul[col[0]][5];
    state[0][2] ^= s_gfMul[col[1]][3];
    state[0][2] ^= s_gfMul[col[2]][4];
    state[0][2] ^= s_gfMul[col[3]][2];
    state[1][2] = s_gfMul[col[0]][2];
    state[1][2] ^= s_gfMul[col[1]][5];
    state[1][2] ^= s_gfMul[col[2]][3];
    state[1][2] ^= s_gfMul[col[3]][4];
    state[2][2] = s_gfMul[col[0]][4];
    state[2][2] ^= s_gfMul[col[1]][2];
    state[2][2] ^= s_gfMul[col[2]][5];
    state[2][2] ^= s_gfMul[col[3]][3];
    state[3][2] = s_gfMul[col[0]][3];
    state[3][2] ^= s_gfMul[col[1]][4];
    state[3][2] ^= s_gfMul[col[2]][2];
    state[3][2] ^= s_gfMul[col[3]][5];
    // Column 4
    col[0] = state[0][3];
    col[1] = state[1][3];
    col[2] = state[2][3];
    col[3] = state[3][3];
    state[0][3] = s_gfMul[col[0]][5];
    state[0][3] ^= s_gfMul[col[1]][3];
    state[0][3] ^= s_gfMul[col[2]][4];
    state[0][3] ^= s_gfMul[col[3]][2];
    state[1][3] = s_gfMul[col[0]][2];
    state[1][3] ^= s_gfMul[col[1]][5];
    state[1][3] ^= s_gfMul[col[2]][3];
    state[1][3] ^= s_gfMul[col[3]][4];
    state[2][3] = s_gfMul[col[0]][4];
    state[2][3] ^= s_gfMul[col[1]][2];
    state[2][3] ^= s_gfMul[col[2]][5];
    state[2][3] ^= s_gfMul[col[3]][3];
    state[3][3] = s_gfMul[col[0]][3];
    state[3][3] ^= s_gfMul[col[1]][4];
    state[3][3] ^= s_gfMul[col[2]][2];
    state[3][3] ^= s_gfMul[col[3]][5];
}

static void PsdkAes_SetupEncrypt(uint8_t const *in, uint8_t *out, const uint32_t *key, int keySize)
{
    uint8_t state[4][4];
    ...
    PsdkAes_AddRoundKey(state, &key[0]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[4]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[8]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[12]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[16]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[20]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[24]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[28]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[32]);
    PsdkAes_SubBytes(state);
    PsdkAes_ShiftRows(state);
    PsdkAes_MixColumns(state);
    PsdkAes_AddRoundKey(state, &key[36]);
    if (keySize != 128) {
        PsdkAes_SubBytes(state);
        PsdkAes_ShiftRows(state);
        PsdkAes_MixColumns(state);
        PsdkAes_AddRoundKey(state, &key[40]);
        PsdkAes_SubBytes(state);
        PsdkAes_ShiftRows(state);
        PsdkAes_MixColumns(state);
        PsdkAes_AddRoundKey(state, &key[44]);
        if (keySize != 192) {
            PsdkAes_SubBytes(state);
            PsdkAes_ShiftRows(state);
            PsdkAes_MixColumns(state);
            PsdkAes_AddRoundKey(state, &key[48]);
            PsdkAes_SubBytes(state);
            PsdkAes_ShiftRows(state);
            PsdkAes_MixColumns(state);
            PsdkAes_AddRoundKey(state, &key[52]);
            PsdkAes_SubBytes(state);
            PsdkAes_ShiftRows(state);
            PsdkAes_AddRoundKey(state, &key[56]);
        } else {
            PsdkAes_SubBytes(state);
            PsdkAes_ShiftRows(state);
            PsdkAes_AddRoundKey(state, &key[48]);
...
void PsdkAes_SetupDecrypt(const uint8_t *in, uint8_t *out, const uint32_t *key, int keySize)
{
    uint8_t state[4][4];
    ...
    if (keySize > 128) {
        if (keySize > 192) {
            PsdkAes_AddRoundKey(state, &key[56]);
            PsdkAes_InvShiftRows(state);
            PsdkAes_InvSubBytes(state);
            PsdkAes_AddRoundKey(state, &key[52]);
            PsdkAes_InvMixColumns(state);
            PsdkAes_InvShiftRows(state);
            PsdkAes_InvSubBytes(state);
            PsdkAes_AddRoundKey(state, &key[48]);
            PsdkAes_InvMixColumns(state);
...
{
    PSDK_ASSERT(pInData != NULL && pOutData != NULL && key != NULL && dataLen % 16 == 0 &&
                      (keyBitSize == 128 || keyBitSize == 192 || keyBitSize == 256));
    uint32_t key_schedule[60];
    uint16_t i;

    PsdkAes_KeySetup(key, key_schedule, keyBitSize);

    for (i = 0; i < dataLen; i += 16) {
        PsdkAes_SetupEncrypt(&pInData[i], &pOutData[i], key_schedule, keyBitSize);
    }
}
...
PsdkAes_Decrypt(uint8_t const *pInData, uint8_t *pOutData, uint16_t dataLen, uint8_t const *key, uint16_t keyBitSize)
{
    ...
    for (i = 0; i < dataLen; i += 16) {
        PsdkAes_SetupDecrypt(&pInData[i], &pOutData[i], key_schedule, keyBitSize);
    }
}
