...
void PsdkTestTask(void const *parameter)
{
    ...
    while (1) 
    {
        if(2 == signalOrAutoModelFlag)         /*2=signal模式*/ 
        {
            xinhaoTable[0] = 'S';
            xinhaoTable[1] = 'I';
            xinhaoTable[2] = 'G';
        ...
        else if(4 == signalOrAutoModelFlag)    /*2s自动拍模式*/
        {
            xinhaoTable[0] = 'a';
            xinhaoTable[1] = 'u';
            xinhaoTable[2] = 't';
        ...
        else                                   /*3s自动拍模式*/
        {
			...
			&bukepaizhaoSp[0],&fuhaoTable[0],ds18b20ToShow,&xinhaoTable[0],ppsRtkHealthNum,
			tim5CatchNum,photoPicNum[1],photoPicNum[2],photoPicNum[3],photoPicNum[4],photoPicNum[5]);
...
/*************************************************
Function: void  PPS_TASK (void const * argument)
Description: 用于存储热靴到来时的拍照位置信息
Return: 无返回值
*************************************************/
uint32_t triggertime = 0;                     /* 接收当前系统时间值 */
T_PsdkPositioningGetPosReq bj_timer = {0};    /* 存储北京时间结构体 */
u8 g_gps003Table[200] = {0};                  /* 存储上次的pos点 */
psdk_f64_t Relative_hight = 0;                /* 存储相对高度 */
void  PPS_TASK (void const * argument)
{
	  uint8_t j = 0,i = 0,writebit = 0,exitbit = 1,flag1 = 1;
