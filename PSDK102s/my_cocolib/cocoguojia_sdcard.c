...
#define MYDISK0  "0:"  /*我们定义 磁盘为0 我们都是对这个磁盘进行操作的*/
...
BYTE  g_cocoguojia_sdcard_myReadBuffer1024[1024] = {0};  /*读缓冲区 如果不读 这个可以屏蔽*/
#endif
...
u8 g_gpsTableFlag = 1;
u8 g_gps001Table[200] = {0};
u8 g_gps002Table[200] = {0};
uint8_t len = 0;
...
void cocoguojia_sdWrireGpsRtk(void)
{
    ...
    if(180>=len)
    {
        ...
        for(i=5;i<len+5;i++)
        {
            ...
            str++;   /*而p++ 就是把 p里面的内容+1个单位，因为 p存的是地址，所以是 地址+1，
					            也就是下一个地址。那么指针p 就指向了 下一个地址了*/
        }             
...
void write_system_version_number(const TCHAR *fileName,const TCHAR *txt__name)
{
	...
	if (f_opendir(&dirs, fileName) == FR_OK) /*打开文件夹*/
	{    
		     sprintf(name,"%s/%s",fileName,txt__name);
		     myResetFlash = f_open(&myFile,name,FA_OPEN_ALWAYS|FA_READ|FA_WRITE);		
	...
	if(myResetFlash == FR_OK)
	{
		f_lseek(&myFile,0);
	  				 l1ll:
					myResetFlash=f_write(&myFile,SYS_VER_NUB,5,&myFileNum);/*将指定存储区内容写入到文件内*/
					if(myResetFlash==FR_OK)
					{
						    ;/*成功*/
					}
...
/*读取SD卡中指定目录是否有.bin文件*/
TCHAR  g_cocoguojia_sdcard_bin[20] ={"SYSTEM/SHARE.bin"};	
int  read_bin(const TCHAR *fileName)
{
			taskENTER_CRITICAL();         
			cocoguojia_RegisteredWorkspace();       /*在FatFs模块上注册一个工作区(文件系统对象)  创建一个磁盘MYDISK0*/
			taskEXIT_CRITICAL(); 
		...
		{    
				...
				myResetFlash = f_open(&myFile, g_cocoguojia_sdcard_bin, FA_OPEN_EXISTING | FA_READ); 
