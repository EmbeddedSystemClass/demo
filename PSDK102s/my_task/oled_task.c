  ...
  osDelay(10);	
	...
	{	   
      ...
      else
      {
			...
			if(OLEDSHOWSTATE_LOGO==oledStateFlag)
			{
				cocoguojia_oledShowShareuavLogoAndID();
			}		

			if( (SYSTEMSTATE_OFFTRUE==oledStateFlag)||(SYSTEMSTATE_ONTRUE==oledStateFlag) )
			{
				if((10<n)&&(20>=n))
				{		
				...
				{
					if(SYSTEMSTATE_ONTRUE==oledStateFlag)
									{
			 ...
			 if(OLEDSHOWSTATE_REPAIRMODEL1==oledStateFlag)
			{
				  cocoguojia_oledShowRepairModel(1);
			}
			 if(OLEDSHOWSTATE_REPAIRMODEL2==oledStateFlag)
			{
				  cocoguojia_oledShowRepairModel(2);
			}
			 if(OLEDSHOWSTATE_REPAIRMODEL3==oledStateFlag)
			{
				 cocoguojia_oledShowRepairModel(3);
			}
			 if(OLEDSHOWSTATE_REPAIRMODEL4==oledStateFlag)
			{
			...
			if(OLEDSHOWSTATE_INIT==oledStateFlag)        //显示 系统初始化中
			{
				cocoguojia_oledShowSystemInit();
			}
			
			if(SYSTEMSTATE_GOTOSHUT==oledStateFlag)     //显示 关机中
			{
				cocoguojia_oledShowShuting();
			}
		
			if(SYSTEMSTATE_GOTOOPEN==oledStateFlag)     //显示 开机中
			{
				cocoguojia_oledShowOpening();
			}
		
			if(SYSTEMSTATE_SHUTFAIL==oledStateFlag)     //关机失败
			{
				cocoguojia_oledShowShutWrong();
			}
		
			if(SYSTEMSTATE_OPENFAIL==oledStateFlag)     //开机失败
			{
				cocoguojia_oledShowOpenWrong();
			}	
        
      if(SYSTEMSTATE_CAMLOSE==oledStateFlag)     //开机失败
			{
				cocoguojia_oledShowLoseWrong();
			}	
					 
			if(OLEDSHOWSTATE_SETMODEL1==oledStateFlag)    //进入到设置模式 page1
			{
				cocoguojia_oledShowSetModel1();
			}
        
        //if(OLEDSHOWSTATE_SETMODEL2==oledStateFlag)    //进入到设置模式 page2
			//{
			//	cocoguojia_oledShowSetModel22();
			//}
		
			if(OLEDSHOWSTATE_SETMODEL3==oledStateFlag)    //进入到设置结果显示界面
			{
				cocoguojia_oledShowSetModel2(0);
			}
			
			if(OLEDSHOWSTATE_SETMODEL4==oledStateFlag)    //进入到当前设置显示界面
			{
