...
#if PSDK_ARCH_USE_OS
#else
#define PSDK_RECEIVE_BUFFER_SIZE    512
#endif
...
#if PSDK_ARCH_USE_OS
                psdkStat = PsdkSys_SemaphorePost(&protHandle->semaphoreWaitAck);
                if (psdkStat != PSDK_STAT_OK) {
                    return psdkStat;
                }
#else
                protHandle->isReceivedAck = 1;
#endif
            }
        }
...
{
    E_PsdkStat psdkStat = PSDK_STAT_OK;

    PSDK_UNUSED(psdkStat);
    ...
    protHandle->receiveCallback = NULL;

    //send
    protHandle->sendSeqNum = 0;
    memset(protHandle->sendFrameBuf, 0x00, sizeof(protHandle->sendFrameBuf));
    memset(&protHandle->sendCmdInfo, 0x00, sizeof(protHandle->sendCmdInfo));
    ...
    memset(&protHandle->ackInfo, 0x00, sizeof(protHandle->ackInfo));
    memset(protHandle->ackData, 0x00, sizeof(protHandle->ackData));
    ...
    psdkStat = PsdkSys_MutexNew(&protHandle->mutexSendGetAck);
    if (psdkStat != PSDK_STAT_OK) {
        return psdkStat;
    }

    psdkStat = PsdkSys_SemaphoreNew(&protHandle->semaphoreWaitAck, 1);
    if (psdkStat != PSDK_STAT_OK) {
        return psdkStat;
    }

    psdkStat = PsdkSys_SemaphoreTimedWait(&protHandle->semaphoreWaitAck, 0);
    if (psdkStat != PSDK_STAT_OK) {
        return psdkStat;
    }
#else
    protHandle->isReceivedAck = 0;
#endif
...
E_PsdkStat PsdkProto_DeInit(T_PsdkProt *protHandle)
{
    ...
    psdkStat = PsdkSys_MutexDestroy(&protHandle->mutexSendGetAck);
    if (psdkStat != PSDK_STAT_OK) {
        return psdkStat;
    }

    psdkStat = PsdkSys_SemaphoreDestroy(&protHandle->semaphoreWaitAck);
    if (psdkStat != PSDK_STAT_OK) {
        return psdkStat;
    }
#endif
...
E_PsdkStat PsdkProto_ProcessReceiveData(T_PsdkProt *protHandle, const uint8_t *pData, uint16_t len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        } else if (parseRes == -ERRCODE_PSDK_PROT_PARSE_FRAME_LENGTH) {
...
/**
 * @brief Register PSDK frame receive callback function to PSDK protocol handle structure.
 * @param protHandle Pointer to PSDK protocol handle.
 * @param callbackFunc Data receive callback function.
 * @return PSDK function process state.
 */
E_PsdkStat PsdkProto_RegReceiveDataFunc(T_PsdkProt *protHandle, PsdkReceiveCallbackFunc callbackFunc)
{
    if (callbackFunc == NULL) {
        return PSDK_STAT_ERR_PARAM;
    }

    protHandle->receiveCallback = callbackFunc;
...
/**
 * @brief PSDK send request data and get ack function.  。PSDK发送请求数据，获取ack功能。
 * @note If system is based on OS, command handle task/thread can not call this function, otherwise system may hang.upauonaue如果系统基于操作系统，命令处理任务/线程不能调用此函数，否则系统可能挂起
 * @note If system is noe based on OS (bare metal system), command calling this function to send will fail when ack of
                                ...
                                T_PsdkProtInfo *ackInfo, uint8_t *ackData, uint32_t timeout)
{
    ...
    PSDK_UNUSED(tempTick);
    PSDK_UNUSED(receiveBuffer);
    PSDK_UNUSED(receiveDataLen);

#if PSDK_ARCH_USE_OS
    if (PsdkSys_MutexLock(&protHandle->mutexSendGetAck) != PSDK_STAT_OK) {
        return PSDK_STAT_ERR;
    }
#else
    if (protHandle->isWaitAck != 0)
        return PSDK_STAT_ERR;
...
#if PSDK_ARCH_USE_OS
    if (PsdkSys_SemaphoreTimedWait(&protHandle->semaphoreWaitAck, timeout) == PSDK_STAT_OK) {
    ...
    protHandle->isWaitAck = 0;
    if (PsdkSys_MutexUnlock(&protHandle->mutexSendGetAck) != PSDK_STAT_OK) {
        return PSDK_STAT_ERR;
    }
#else
    receiveBuffer = PSDK_MEM_MALLOC(PSDK_RECEIVE_BUFFER_SIZE);
    if (receiveBuffer == NULL) {
        protHandle->isWaitAck = 0;
        return PSDK_STAT_ERR_ALLOC;
    }

    if (protHandle->receiveCallback != NULL) {
        tempTick = PSDK_GET_TIME_MS;
        do {
            receiveDataLen = protHandle->receiveCallback(receiveBuffer, PSDK_RECEIVE_BUFFER_SIZE);
            PsdkProto_ProcessReceiveData(protHandle, receiveBuffer, receiveDataLen);
        } while ((PSDK_GET_TIME_MS - tempTick) < timeout && protHandle->isReceivedAck == 0);
    } else {
        psdkStat = PSDK_STAT_ERR;
    }

    PSDK_MEM_FREE(receiveBuffer);
    protHandle->isWaitAck = 0;

    if (protHandle->isReceivedAck == 1) {
        *ackInfo = protHandle->ackInfo;
        memcpy(ackData, protHandle->ackData, protHandle->ackInfo.dataLen);
        return PSDK_STAT_OK;
    }
