...
#define EMU_SD_FORMAT_TIME_COST     500
...
static uint32_t formatStTimeMs = 0;
static uint8_t isFormatting = 0;
...
void CamEmu_SetCameraMode(const T_PsdkCameraSetModeReq *req, T_PsdkCameraSetModeAck *ack)
{
    ...
    if ((req->cameraMode == PSDK_CAMERA_CAMERA_MODE_RECORD_VIDEO
         && isShootingBurstPhoto == 0
         && photographyIsOnInterval == 0)
        || (req->cameraMode == PSDK_CAMERA_CAMERA_MODE_SHOOT_PHOTO
            && recordingState == 0))
...
//    PSDK_LOG_DEBUG("Get ShootPhoto State");
    ...
    if (photographyIsOnInterval == 1) {
        ...
        if (tmp <= EMU_SINGLE_TIME_COST) {
            ...
            isShootingSinglePhoto = 0;
						capture_FinishAndStartPhotoFlag=0;
        }

        if (intervalCount != 255 &&
            (OS_GET_TIME_MS - ShotStTimeMs) > ((intervalCount - 1) * intervalTime * 1000 + EMU_SINGLE_TIME_COST))
            photographyIsOnInterval = 0;
    } else {
        //other
...
//    PSDK_LOG_DEBUG("Get Recording State");
    ...
    if (recordingState == 1) {
        ...
        ack->currentRecordingTimeInSeconds = 0;
    }
...
//    PSDK_LOG_DEBUG("Get Sd card Param");
    ...
    if (isFormatting == 1) {
        if ((OS_GET_TIME_MS - formatStTimeMs) < EMU_SD_FORMAT_TIME_COST) {
            ack->isFormatting = 1;
        } else {
            isFormatting = 0;
            ack->isFormatting = 0;
...
//    PSDK_LOG_DEBUG("GetOpticalZoomFactor");
    ...
    opticalFactor = (psdk_f32_t) ((currentFocalLength - minFocalLength) / 10.0 + 1.0);
