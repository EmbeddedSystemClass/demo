...
u16 g_sdioFac_ms=0;//ms延时倍乘数
static u8  sdioFac_us=0;//us延时倍乘数

//初始化延迟函数
//当使用ucos的时候,此函数会初始化ucos的时钟节拍
//SYSTICK的时钟固定为HCLK时钟的1/8
//SYSCLK:系统时钟
void SdioDelayInit(void)	 
{

	sdioFac_us=SystemCoreClock/8000000;	//为系统时钟的1/8  
	g_sdioFac_ms=(u16)sdioFac_us*1000;//非ucos下,代表每个ms需要的systick时钟数   
}	

  
/* Private functions ---------------------------------------------------------*/
/*
 * 函数名：cocoguojia_sdDeInit
 * 描述  ：复位SDIO端口
 * 输入  ：无
 * 输出  ：无
 */
void cocoguojia_sdDeInit(void)
{
  ...
  GPIO_InitStructure.GPIO_Pin = SDIO_D0_IO | SDIO_D1_IO | SDIO_D2_IO | SDIO_D3_IO | SDIO_CLK_IO;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
  GPIO_Init(SDIO_Dx_CLK_PORT, &GPIO_InitStructure);

  //Configure PD.02 CMD line
  GPIO_InitStructure.GPIO_Pin = SDIO_CMD_IO;
...
/**
  * @brief  Returns the DMA End Of Transfer Status.  //返回传输状态的DMA结束。
  * @param  None
...
static void GPIO_Configuration(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_Dx_CLK_PORT | RCC_APB2Periph_CMD_PORT , ENABLE);
...
SD_Error SD_Init(void)
{
  ...
  //卡识别成功，进行卡初始化 
  errorstatus = SD_InitializeCards();	//初始化所有的卡或者单个卡进入就绪状态 返回SD_OK (SD_OK值为0)则表示没错误  返回其它表示有错误
  ...
  SDIO_InitStructure.SDIO_ClockDiv 				= SDIO_TRANSFER_CLK_DIV; 					//时钟分频系数定位1 则CLK时钟线的速度为24MHz SDIOCLK = HCLK, SDIO_CK = HCLK/(2 + SDIO_TRANSFER_CLK_DIV) 
  SDIO_InitStructure.SDIO_ClockEdge 			= SDIO_ClockEdge_Rising;					//CLK 上升沿有效 采集数据 
  SDIO_InitStructure.SDIO_ClockBypass 			= SDIO_ClockBypass_Disable;					//禁止时钟分频旁路 时钟频率若超过24M,要开启此模式 	
  SDIO_InitStructure.SDIO_ClockPowerSave 		= SDIO_ClockPowerSave_Disable;				//禁止节能模式  若开启此功能，在总线空闲时关闭sd_clk时钟
  SDIO_InitStructure.SDIO_BusWide 				= SDIO_BusWide_1b;							//数据线宽度 暂时配置成1bit模式
  SDIO_InitStructure.SDIO_HardwareFlowControl 	= SDIO_HardwareFlowControl_Disable; 		//禁止硬件流 若开启则在FIFO不能进行发送和接收数据时，数据传输暂停
  SDIO_Init(&SDIO_InitStructure);
  
  
  if (errorstatus == SD_OK)//如果之前的各个函数的返回值errorstatus=SD_OK 即没错误则
  {
		errorstatus = SD_GetCardInfo(&SDCardInfo);//用来读取csd/cid寄存器 
	    //CID 卡识别号(用来识别卡的个体号码 具有唯一性)16个字节
	    //CSD 卡的特定信息(卡的操作条件信息)16个字节
  }

  if (errorstatus == SD_OK)//如果之前的各个函数的返回值errorstatus=SD_OK 即没错误则
  {
    errorstatus = SD_SelectDeselect((uint32_t) (SDCardInfo.RCA << 16));	// 通过cmd7  rca选择要操作的卡
	//rca相对地址 卡的本地系统地址 初始化时	 动态地由卡建议 主机核准 
  }

  if (errorstatus == SD_OK)//如果之前的各个函数的返回值errorstatus=SD_OK 即没错误则
  {
			
    errorstatus = SD_EnableWideBusOperation(SDIO_BusWide_4b);	//最后为了提高读写，开启4bits模式
  }  

  return(errorstatus);//返回errorstatus
}
...
//此函数将等待SDIO DMA数据传输完成。
//*这个函数应该在SDIO_ReadMultiBlocks()函数之后调用
