                                                ...
                                                const uint8_t *inProtData)
{
    ...
    if (ack.prop.widgetType == PSDK_APPFUNC_WIDGET_TYPE_LIST) {
        memset(ack.prop.widgetAttribute.listAttribute.listItemName, 0x00,
               sizeof(ack.prop.widgetAttribute.listAttribute.listItemName));
        ...
        for (int i = 0; i < ack.prop.widgetAttribute.listAttribute.listItemNum; i++) {
            strncpy((char *) ack.prop.widgetAttribute.listAttribute.listItemName[i],
                    s_widgetList[sub].widgetAttribute.listAttribute.listItemName[i],
        ...
        memset(ack.prop.widgetAttribute.intInputBoxAttribute.promptChar, 0x00,
               sizeof(ack.prop.widgetAttribute.intInputBoxAttribute.promptChar));
        strncpy((char *) ack.prop.widgetAttribute.intInputBoxAttribute.promptChar,
                s_widgetList[sub].widgetAttribute.intInputBoxAttribute.promptStr,
...
/**
 * @brief Handle Get Text Input Box Param Command.
 * @param protHandle Pointer to PSDK protocol handle.
 * @param inProtInfo Pointer to PSDK protocol information of data package.
 * @param inProtData Pointer to data field of package.
 * @return PSDK function process state.
 */
static E_PsdkStat AppFuncCmd_GetTextInputBoxParam(struct _PsdkProt *protHandle, const T_PsdkProtInfo *inProtInfo,
                                                  const uint8_t *inProtData)
{
    T_PsdkAppFuncGetTextInputBoxParamAck ack;
    E_PsdkAppFuncTextInputBoxDisplayControlFlag textInputBoxDisplayControlFlag = PSDK_APPFUNC_TEXT_INPUT_BOX_NOT_DISPLAY;

    PSDK_UNUSED(inProtData);

    if (s_getTextInputBoxParamFunc != NULL) {
        if (s_getTextInputBoxParamFunc(&textInputBoxDisplayControlFlag, ack.boxName, ack.boxDescription) ==
            PSDK_STAT_OK) {
            ack.ackCode = PSDK_CMD_ACK_CODE_OK;
            ack.displayControlFlag = textInputBoxDisplayControlFlag;
        } else {
            ack.ackCode = PSDK_CMD_ACK_CODE_ERROR;
        }
    } else {
    ...
    PsdkProto_SendAckData(protHandle, inProtInfo, (uint8_t *) &ack, sizeof(T_PsdkAppFuncGetTextInputBoxParamAck));
    return PSDK_STAT_OK;
}
...
static const T_PsdkProtCmdItem s_appFuncProtCmdList[] =
        {
                ...
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_APP_FUNC, PSDK_APPFUNC_CMDID_GET_WIDGET_LIST_LENGTH,
                                   AppFuncCmd_GetWidgetListLength),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_APP_FUNC, PSDK_APPFUNC_CMDID_GET_WIDGET_LIST_VALUE,
                                   AppFuncCmd_GetWidgetListValue),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_APP_FUNC, PSDK_APPFUNC_CMDID_GET_WIDGET_GROUP_COUNT,
                                   AppFuncCmd_GetWidgetGroupCount),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_APP_FUNC, PSDK_APPFUNC_CMDID_GET_WIDGET_GROUP_VALUE,
                                   AppFuncCmd_GetWidgetGroupValue),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_APP_FUNC, PSDK_APPFUNC_CMDID_SET_WIDGET_VALUE,
                                   AppFuncCmd_SetWidgetValue),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_APP_FUNC, PSDK_APPFUNC_CMDID_GET_TEXT_INPUT_BOX_PARAM,
 ...
 * @note Max widget count is ::APPFUNC_MAX_WIDGET_COUNT.
 * @param psdkUpper Pointer to the top-level PSDK structure.
...
/**
 * @brief Register the callback function for getting text input box parameters.
 * @param psdkUpper pointer to the top-level PSDK structure.
 * @param getTextInputBoxParamFunc the callback function for getting text input box parameters.
 * @return an enum that represents a status of PSDK.
 */
E_PsdkStat PsdkAppFunc_RegGetTextInputBoxParamCallback(T_PsdkUpper *psdkUpper,
                                                       GetTextInputBoxParamFunc getTextInputBoxParamFunc)
{
    PSDK_UNUSED(psdkUpper);

    s_getTextInputBoxParamFunc = getTextInputBoxParamFunc;
