 ...
 * @param psdkUpper Pointer to the top-level PSDK data structure.
 * @param sendCallbackFunc Pointer to the external send function.
...
/**
 * @brief Register the receive function for PSDK, which is externally provided by the user.
 * @param psdkUpper Pointer to the top-level PSDK data structure.
 * @param receiveCallbackFunc Pointer to the external receive function.
 * @return An enum that represents a status of PSDK.
 */
E_PsdkStat PsdkUpper_RegReceiveFunction(T_PsdkUpper *psdkUpper, PsdkReceiveCallbackFunc receiveCallbackFunc)
{
    return PsdkProto_RegReceiveDataFunc(&(psdkUpper->protHandle), receiveCallbackFunc);
}
