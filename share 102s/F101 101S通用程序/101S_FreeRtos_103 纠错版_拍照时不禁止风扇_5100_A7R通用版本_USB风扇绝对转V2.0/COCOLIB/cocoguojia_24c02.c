...
//g_pwmOrTtlModelFlag=0;					//0=pwm   1=ttl-H   2=ttl-L
...
void cocoguojia_readSetParameter(void)
{
	...
	cocoguojia_eeReadBytes(&readbuf[0],PARAMETERADDR,3);
	if(YANZHENGDATA==readbuf[0])
	{
		g_pwmOrTtlModelFlag=readbuf[1];
	}
	else
	{
		...
		readbuf[1]=0;
		g_pwmOrTtlModelFlag=readbuf[1];
		cocoguojia_eeWriteBytes(&readbuf[0],PARAMETERADDR,3);
	}	
