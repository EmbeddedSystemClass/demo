...
{ 	 
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA|RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD, ENABLE);	 //使能PC,D,G端口时钟
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15;
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		//速度50MHz
 	GPIO_Init(GPIOA, &GPIO_InitStructure);	  				//初始化GPIOD3,6
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_12;	 //PD3,PD6推挽输出  
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		//速度50MHz
...
void cocoguojia_oledShowSetModel1(void)
{
	...
	{
		OLED_ShowStringFanBai(0,2,"PWM  ");//一共可以写16个字符
	}
	else
	{
		OLED_ShowString(0,2,"PWM  ");//一共可以写16个字符
	}
	
	if(4==g_currentRowNum)
	{
		OLED_ShowStringFanBai(0,4,"TTL-H ");//一共可以写16个字符
	}
	else
	{
		OLED_ShowString(0,4,"TTL-H  ");//一共可以写16个字符
	}

	if(6==g_currentRowNum)
	{
		OLED_ShowStringFanBai(0,6,"TTL-L  ");//一共可以写16个字符
	}
	else
	{
		OLED_ShowString(0,6,"TTL-L  ");//一共可以写16个字符
	}	    
}


//--------------------------------------------------------
//curr=1 显示当前设置  curr=0 显示设置结果
void cocoguojia_oledShowSetModel3(uint8_t curr)
{
	...
	else
	{
		...
		{
			OLED_ShowStringFanBai(0,2,"TTL-H  ");//一共可以写16个字符
		}
		else
		{
			OLED_ShowStringFanBai(0,2,"TTL-L  ");//一共可以写16个字符
		}
	}	
}
