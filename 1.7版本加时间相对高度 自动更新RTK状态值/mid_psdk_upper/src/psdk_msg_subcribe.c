...
/**
 * @brief Receive data transmission bandwidth message.
...
/**
 * @brief Receive UTC timestamp data.
 * @param protHandle Pointer to PSDK protocol handle.
 * @param inProtInfo Pointer to PSDK protocol information of data package.
 * @param inProtData Pointer to data field of package.
 * @return PSDK function process state.
                                          ...
                                          const T_PsdkProtInfo *inProtInfo, const uint8_t *inProtData)
{
    ...
    if (s_PushDataMsgCallbackList.utcTimestampMsgCallback != NULL) {
        s_PushDataMsgCallbackList.utcTimestampMsgCallback((const T_UtcTimestampMsg *) inProtData);
    }
    return PSDK_STAT_OK;
}

/**
 * @brief Receive type of other payload mounted on UAV.
 * @param protHandle Pointer to PSDK protocol handle.
 * @param inProtInfo Pointer to PSDK protocol information of data package.
 * @param inProtData Pointer to data field of package.
 * @return PSDK function process state.
 */
static E_PsdkStat MsgPushCmd_OtherPayloadType(struct _PsdkProt *protHandle, const T_PsdkProtInfo *inProtInfo,
                                              const uint8_t *inProtData)
{
    PSDK_UNUSED(protHandle);
    PSDK_UNUSED(inProtInfo);

    if (s_PushDataMsgCallbackList.otherPayloadTypeMsgCallback != NULL) {
        s_PushDataMsgCallbackList.otherPayloadTypeMsgCallback((const T_OtherPayloadTypeMsg *) inProtData);
    }
    return PSDK_STAT_OK;
}

/**
 * @brief Receive focal length of other payload mounted on UAV.
 * @param protHandle Pointer to PSDK protocol handle.
 * @param inProtInfo Pointer to PSDK protocol information of data package.
 * @param inProtData Pointer to data field of package.
 * @return PSDK function process state.
 */
static E_PsdkStat MsgPushCmd_OtherPayloadFocalLength(struct _PsdkProt *protHandle, const T_PsdkProtInfo *inProtInfo,
                                                     const uint8_t *inProtData)
{
    PSDK_UNUSED(protHandle);
    PSDK_UNUSED(inProtInfo);

    if (s_PushDataMsgCallbackList.otherPayloadFocalLengthMsgCallback != NULL) {
        s_PushDataMsgCallbackList.otherPayloadFocalLengthMsgCallback((const T_OtherPayloadFocalLengthMsg *) inProtData);
    }
    return PSDK_STAT_OK;
}
...
static const T_PsdkProtCmdItem s_PushDataMsgSubProtCmdList[] =
        {
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_BANDWIDTH,
                ...
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_BATTERY_INFO,
                ...
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_APP_DATETIME,
                                   MsgPushCmd_PushAppDateTime),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_BARO_HEIGHT_DATA,
                                   MsgPushCmd_PushBaroHeight),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_GPS_RAW_DATA,
                                   MsgPushCmd_GpsRawData),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_RTK_RAW_DATA,
                                   MsgPushCmd_RtkRawData),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_UTC_TIMESTAMP,
                                   MsgPushCmd_UtcTimestamp),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_OTHER_PAYLOAD_TYPE,
                                   MsgPushCmd_OtherPayloadType),
                PSDK_PROT_CMD_ITEM(PSDK_CMDSET_DATA_PUSH, PSDK_DATAPUSH_CMD_ID_PUSH_OTHER_PAYLOAD_FOCAL_LENGTH,
 ...
 */
E_PsdkStat
PsdkMsgSub_RegMsgCallbackList(T_PsdkUpper *psdkUpper, const T_PsdkPushDataMsgCallbackList *PushDataMsgCallbackList)
{
